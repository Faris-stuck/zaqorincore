"""Stub DNS intel collector for T1583.001 detection.
...
class CollectorConfig:
    ...
    Attributes:
        whoid_rdap_url: Backend URL for RDAP/WHOIS lookups. Operators
...
def load_config() -> CollectorConfig:
    ...
    return CollectorConfig(
        whoid_rdap_url=os.environ.get("WHOIS_RDAP_URL"),
...
def collect_once(_config: CollectorConfig) -> int:
    ...
    Returns the number of enriched events emitted. The stub returns 0
    because the real collection happens in the Go agent. Tests that
    import this module assert the stub is wired correctly without
    exercising any network code.
    """
