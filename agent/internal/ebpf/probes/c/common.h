...
// Pull kernel integer types (__u32, __u64, __s32, ...) into
// the BPF compile context. Required because bpf_helpers.h
// (and vmlinux.h, when used) reference these types and the
// BPF target does not pull in linux/types.h by default.
#include <linux/types.h>

// Minimal trace_event_raw_sys_enter stub matching the
// tracepoints we attach to. Full vmlinux.h would do the
// same job but adds ~10k lines; this struct is what every
// SEC("tracepoint/syscalls/sys_enter_*") handler receives.
...
// Socket address families. Defined here (rather than via
// <sys/socket.h>) to avoid pulling in glibc's gnu/stubs-32.h
// during BPF compilation. The numeric values match POSIX.
#define AF_INET  2
#define AF_INET6 10
