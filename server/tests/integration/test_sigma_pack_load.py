...
def test_load_rules_from_dir_returns_18():
    """Round 13 audit claim: 18 rules in the self-defense pack.

    The loader uses ``*.yml`` + ``*.yaml`` rglob and returns
    CompiledSigmaRule instances. We assert exactly 18.
    """
...
def test_load_rules_level_distribution():
    """Round 13 audit claim: 10 high, 7 medium, 1 low, 0 critical."""
    rules = load_rules_from_dir(SELF_DEFENSE_DIR)
    levels = {}
    for r in rules:
        levels[r.level] = levels.get(r.level, 0) + 1
    assert levels.get("high", 0) == 10, f"high count: {levels}"
